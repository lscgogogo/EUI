<template>
  ...
  <template v-if="modelValue">
    <Teleport to="body">
      <!--  遮罩层-->
      <div class="e-dialog-overlay" @click="clickOverlay"></div>
      <div class="e-dialog">
        <header class="e-dialog-header">
          <h3>{{ title }}</h3>
          <span @click="close" class="e-dialog-close"></span>
        </header>
        <!--  dialog主要内容-->
        <main class="e-dialog-content">
          <slot name="content"/>
        </main>
        <!--    按钮-->
        <footer>
          <e-button @click="handleCancel">Cancel</e-button>
          <e-button level="main" @click="handleConfirm">Confirm</e-button>
        </footer>
      </div>
