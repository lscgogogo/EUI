<template>
  <div ref="selectRef" class="e-select" :class="selectSizeClass" v-click-outside="handleClose"
       @click.stop="selectBoxClicked">
    <!--        标签tag-->
    <div ref="selectedTagRef" class="e-selected-tags">
      <e-tag closable v-for="item in selectedArr" type="info" :key="item" @close="updateSelectedItem(item)">{{ item }}</e-tag>
    </div>
    <div class="e-input">
      <div class="e-input-wrapper" :class="{'is-focus':isFocus}">

        <!--        输入框-->
        <input ref="selectInput" class="e-input-inner" type="text" :value="modelValue" readonly autocomplete="off"
               :placeholder="modelValue?'':placeholder">
        <span class="e-input-suffix-box" :class="{'is-expand':popoverVisible}">
    ...
    <!--  下拉弹窗  -->
      <div ref="popoverBoxRef" class="e-select-popover" :class="popoverComputedClass" v-if="popoverVisible">
        <div class="contentBox">
          <div class="e-select-item-wrapper">
            <ul>
              <slot></slot>
            </ul>
          </div>
          <span class="e-select-dropdown-arrow"></span>
        </div>
      </div>
  </div>
</template>
...
const selectSizeClass = computed(() => {
  return {
    [`e-select-${props.size}`]: props.size
  };
});
