<template>
  <div class="e-carousel" ref="rootRef" @mouseover="showArrow" @mouseleave="hideArrow">
    <div ref="carouselContainerRef" class="e-carousel-container" :style="containerStyle">
      <!--    左右两边的按钮-->
      <transition name="leftArrow">
        <button v-show="(isArrowShow&&direction!=='vertical'&&props.arrow!=='never')||props.arrow==='always'"
                class="e-carousel-arrow e-carousel-arrow-left"
                @click="prev">
          <svg class="icon">
      ...
      <transition name="rightArrow">
        <button v-show="(isArrowShow&&direction!=='vertical'&&props.arrow!=='never')||props.arrow==='always'"
                class="e-carousel-arrow e-carousel-arrow-right"
                @click="next">
          <svg class="icon">
    ...
    <!--    下方定位条-->
    <ul ref="carouselIndicatorRef" class="e-carousel-indicators" :class="indicatorClass">
      <li class="e-carousel-indicator" v-for="(item,index) in slots"
...
const indicatorClass = computed(() => {
  return {
    [`e-carousel-indicators-${props.direction}`]: props.direction,
    [`e-carousel-indicators-pos-${props.indicatorPosition}`]: props.indicatorPosition
  };
});
...
.e-carousel {
  ...
  .e-carousel-container {
    ...
    .e-carousel-arrow {
      ...
      &.e-carousel-arrow-left {
        left: 16px;
      }

      &.e-carousel-arrow-right {
        right: 16px;
      }
  ...
  .e-carousel-indicators {
    ...
    &.e-carousel-indicators-pos-none {
      display: none;
    }

    &.e-carousel-indicators-pos-outside {
      position: relative;
      display: flex;
      justify-content: center;
      align-items: center;

      &.e-carousel-indicators-vertical {
        top: 0;
        left: 2px;
        transform: none;
    ...
    &.e-carousel-indicators-horizontal {
      left: 50%;
      bottom: 0;
